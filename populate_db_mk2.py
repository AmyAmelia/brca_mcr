...
# for i in range(df.shape[0]):
#     # pull variant annotations from myvaraint api
#     mv = myvariant.MyVariantInfo()
#     var_ann = mv.getvariant('chr17:' + (df.iloc[i]['Variant Genome']), ['dbsnp.rsid', 'cadd.polyphen.cat', 'cadd.sift.cat', 'gnomad_exome.af.af'])
#     if var_ann is None:
#         rsID_ann= "N/A"
...
#         else:
#             gnomad_ann = "N/A"
#     # print (rsID_ann, polyphen_ann, sift_txt, gnomad_ann)
#     # print ()

#     #  Add to annotation table. 
#     record = Annotation.objects.create(rsid = rsID_ann, polyphen = polyphen_ann, sift = sift_ann, gnomad_af = gnomad_ann,
#                         id_variant = Variant.objects.filter(genomic=df.iloc[i]['Variant Genome'])[0],)
